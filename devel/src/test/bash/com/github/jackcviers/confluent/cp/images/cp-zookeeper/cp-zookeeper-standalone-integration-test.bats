...
docker_compose_command="docker-compose -f ${PWD}/devel/src/test/bash/com/github/jackcviers/confluent/cp/images/cp-zookeeper/fixtures/standalone-config.yml"
...
setup_file(){
    mkdir -p ./build/tmp/zk-config-kitchen-sink-test/data \
    ...
    ${docker_compose_command} \
	exec kerberos \
	$(kadmin_keytab_create zookeeper \
			       sasl-config zookeeper-config)
    ${docker_compose_command} \
	exec kerberos \
	$(kadmin_keytab_create zkclient \
			       sasl-config zkclient-config)
}
