...
@test "zookeeper on bridge network driver should be accessible from outside the containe" {
    run ${BATS_BUILD_TOOL} run --rm -it --name \
	cp-zookeeper-bridge-network-test ${BATS_IMAGE} \
	--network=host $(health_check localhost 22181)
    assert_output --partial "PASS"
}
...
@test "zookeeper on host network driver should be accessible from outside the container" {
    run ${BATS_BUILD_TOOL} run --rm -it --name \
	cp-zookeeper-bridge-network-test ${BATS_IMAGE} \
	--network=host $(health_check localhost 32181)
    assert_output --partial "PASS"
}

@test "jmx client connection should work" {
    run ${BATS_BUILD_TOOL} run --rm -dt --name cp-jmxterm-test \
	localhost/jackcviers/cp-jmxterm:${VERSION} \
	--network=standalone-network-test_zk \
	$(health_check localhost 32181)
    run ${BATS_BUILD_TOOL} logs cp-jmxterm-test
    assert_output --partial "Version = 3.4.9-1757313, built on 08/23/2016 06:50 GMT"
}
